    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-wrap items-center gap-4 mb-6">
            <UBadge class="total-components-badge px-4 py-2" variant="subtle" size="lg" color="gray">
                Total Components: <span class="font-bold ml-1">{{ componentCodeStore.getActualComponentMap().length
                    }}</span>
            </UBadge>
            <div class="flex gap-2">
                <UButton size="lg" variant="solid" color="white" icon="i-heroicons-clipboard-document-list"
                    @click="logComponents">
                    Log components
                </UButton>
                <UButton class="initialize-components-button" @click="ComponentManager.initializeComponents()" size="lg"
        ...
        <div class="max-h-[800px] overflow-y-auto bg-gray-50 border border-gray-200 p-6 rounded-xl shadow-inner">
                ...
                <div class="p-6 space-y-6 bg-white">
                    ...
                    <div v-if="Object.keys(component.html).length > 0">
                        ...
                        <div v-for="(value, key) in component.html" :key="key" class="mb-4 last:mb-0">
                            ...
                            <div class="relative group">
                                <pre
                                    class="text-xs font-mono bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto border border-gray-800 shadow-inner leading-relaxed">{{ value }}</pre>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="Object.keys(component.css).length > 0">
                        ...
                        <div v-for="(value, key) in component.css" :key="key" class="mb-4 last:mb-0">
                            <p class="text-xs font-semibold text-gray-700 mb-1 font-mono">{{ key }}</p>
                            <pre
                                class="text-xs font-mono bg-slate-900 text-blue-100 p-4 rounded-lg overflow-x-auto border border-slate-800 shadow-inner leading-relaxed">{{ value }}</pre>
                        </div>
                    </div>
                    ...
                    <div v-if="Object.keys(component.js).length > 0">
                        ...
                        <div v-for="(value, key) in component.js" :key="key" class="mb-4 last:mb-0">
                            <p class="text-xs font-semibold text-gray-700 mb-1 font-mono">{{ key }}</p>
                            <pre
                                class="text-xs font-mono bg-gray-900 text-yellow-50 p-4 rounded-lg overflow-x-auto border border-gray-800 shadow-inner leading-relaxed">{{ value }}</pre>
                        </div>
                    </div>
                    ...
                    <div v-if="Object.keys(component.sql).length > 0">
                        ...
                        <div v-for="(value, key) in component.sql" :key="key" class="mb-4 last:mb-0">
                            <p class="text-xs font-semibold text-gray-700 mb-1 font-mono">{{ key }}</p>
                            <pre
                                class="text-xs font-mono bg-gray-900 text-purple-200 p-4 rounded-lg overflow-x-auto border border-gray-800 shadow-inner leading-relaxed">{{ value }}</pre>
                        </div>
                    </div>
            ...
            <div v-if="componentCodeStore.getActualComponentMap().length === 0"
