                    ...
                    <UForm :state="newSession" @submit="handleAddSession(newSession)" class="flex flex-col space-y-4">
                        <div class="highlightable" id="sessions-add-from_date"
                            ...
                            <div class="component-wrapper">
                                ...
                                <input
                                    :class="['form-input', { 'border-red-500': !newSessionFromDateComputed, 'border-sky-500': newSessionFromDateComputed }]"
                                    id="from_date" v-model="newSession.from_date" type="date"
                                    :disabled="highlightStore.isHighlightMode" />
                                <div v-if="newSessionFromDateError" class="text-red-500 text-sm mt-1 font-bold">
                                    {{ newSessionFromDateError }}
                                </div>
                        ...
                        <div class="highlightable" id="sessions-add-to_date"
                            ...
                            <div class="component-wrapper">
                                ...
                                <input
                                    :class="['form-input', {
                                        'border-red-500': !newSessionToDateComputed || !newSessionDateRangeComputed,
                                        'border-sky-500': newSessionToDateComputed && newSessionDateRangeComputed
                                    }]"
                                    id="to_date" v-model="newSession.to_date" type="date"
                                    :disabled="highlightStore.isHighlightMode" />
                                <div v-if="newSessionToDateError" class="text-red-500 text-sm mt-1 font-bold">
                                    {{ newSessionToDateError }}
                                </div>
                        ...
                        <div class="highlightable" id="sessions-add-capacity"
                            ...
                            <div class="component-wrapper">
                                ...
                                <input
                                    :class="['form-input', { 'border-red-500': !newSessionCapacityComputed, 'border-sky-500': newSessionCapacityComputed }]"
                                    id="capacity" v-model="newSession.capacity" type="number" min="1"
                                    placeholder="50" :disabled="highlightStore.isHighlightMode" />
                                <div v-if="newSessionCapacityError" class="text-red-500 text-sm mt-1 font-bold">
                                    {{ newSessionCapacityError }}
                                </div>
                            </div>
                        </div>
...
const newSessionFromDateComputed = computed(() => {
    const value = newSession.value.from_date
    return value && value.trim().length > 0
})

const newSessionToDateComputed = computed(() => {
    const value = newSession.value.to_date
    return value && value.trim().length > 0
})

const newSessionDateRangeComputed = computed(() => {
    const fromDate = newSession.value.from_date
    const toDate = newSession.value.to_date
    return isValidDateRangeFromComponent(fromDate, toDate)
})

// Error message computed properties for new session form
const newSessionFromDateError = computed(() => {
    const value = newSession.value.from_date
    if (!value) {
...
const newSessionToDateError = computed(() => {
    const value = newSession.value.to_date
    if (!value) {
        return 'Datum konce je povinné'
    } else if (!isValidDateRangeFromComponent(newSession.value.from_date, value)) {
        return 'Datum konce musí být stejné nebo pozdější než datum začátku'
    }
    return ''
})

const newSessionCapacityComputed = computed(() => {
    const value = newSession.value.capacity
    return value && value > 0
})

const newSessionCapacityError = computed(() => {
    const value = newSession.value.capacity
    if (!value || value <= 0) {
...
const hasValidationErrors = computed(() => {
    return !!(
        newSessionFromDateError.value ||
        newSessionToDateError.value ||
        newSessionCapacityError.value
    )
})
...
const resetForm = () => {
    ...
    emit('update:addModalOpen', false)
}

const handleAddSession = async (data: any) => {
    // Check for validation errors before submitting
    if (newSessionFromDateError.value || newSessionToDateError.value || newSessionCapacityError.value) {
        toast.add({
